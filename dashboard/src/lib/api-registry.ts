...
export const API_ENDPOINTS: ApiEndpoint[] = [
  ...
  { method: "GET", path: "/api/versions/status", group: "Servers", safe: true, desc: "Per-server version status: pinned line, installed vs latest build (hotfix), latest full version (update-available). Full versions are never auto-applied." },
  { method: "POST", path: "/api/tasks/:id/update", group: "Servers", desc: "Apply a software update: {} = hotfix to the latest build of the pinned line; {version} = explicit full-version switch; {autoUpdate} = toggle auto-hotfix. Jar swap + restart, data untouched.", params: [{ name: "id", example: "network-lobby" }], sampleBody: { hotfix: true } },
  { method: "POST", path: "/api/tasks/:id/resync", group: "Servers", desc: "Re-apply the overlay chain (egg + _global/<kind> + named templates + task) to running instances and restart them. ?restart=0 = files only. PATCH {templateSync:true,templateSyncRestart:false} on the task auto-resyncs on overlay changes.", params: [{ name: "id", example: "network-lobby" }] },
  { method: "GET", path: "/api/global-templates", group: "Servers", safe: true, desc: "Named generic file templates (shared files applied to hand-picked services)." },
  { method: "POST", path: "/api/global-templates", group: "Servers", desc: "Create a named template; files live at overlays/_tpl/<id>/.", sampleBody: { name: "Anti-cheat config", taskIds: [] } },
  { method: "PATCH", path: "/api/global-templates/:id", group: "Servers", desc: "Rename / set member services. ?apply=1 re-syncs files to affected services now.", params: [{ name: "id", example: "anti-cheat-config" }], sampleBody: { taskIds: ["network-lobby", "network-world"] } },
  { method: "DELETE", path: "/api/global-templates/:id", group: "Servers", desc: "Delete a named template and its files.", params: [{ name: "id", example: "anti-cheat-config" }] },
  ...
  { method: "POST", path: "/api/connector/action", group: "Players & Connector", desc: "Queue a player action: move/message/broadcast/kick/unqueue (scoped to the player's server via serverId/env).", sampleBody: { kind: "message", player: "Notch", text: "hello", serverId: "network-lobby-201", env: "server" } },
  ...
  { method: "POST", path: "/api/connector/unregister", group: "Players & Connector", desc: "(plugin) Graceful-shutdown deregistration — panel shows 'restarting…' instantly. Token-auth.", sampleBody: { id: "Lobby-1" } },
  ...
  { method: "GET", path: "/api/audit", group: "Automation", safe: true, query: "player=c4g7&days=7", desc: "Player audit trail (joins/quits/switches + operator actions), DSGVO-retained day files. Omit player for everything." },
  { method: "POST", path: "/api/audit", group: "Automation", desc: "Set the DSGVO audit retention window (days, 1–365; older day files purge daily).", sampleBody: { retentionDays: 30 } },
  { method: "POST", path: "/api/audit/erase", group: "Automation", destructive: true, desc: "DSGVO right-to-erasure: permanently remove ALL audit entries of a player (name or uuid).", sampleBody: { player: "Notch" } },

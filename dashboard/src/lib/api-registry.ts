...
export const API_ENDPOINTS: ApiEndpoint[] = [
  ...
  { method: "POST", path: "/api/tasks/:id/resync", group: "Servers", desc: "Re-apply the overlay chain (overlays/_global/<kind> + overlays/<egg> + tasks/<task>) to running instances and restart them. ?restart=0 = files only. PATCH {templateSync:true} on the task auto-resyncs on overlay changes.", params: [{ name: "id", example: "network-lobby" }] },

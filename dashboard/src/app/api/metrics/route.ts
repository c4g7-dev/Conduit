import { NextResponse } from "next/server";
import { getDB } from "@/lib/store";
import { blueprint } from "@/lib/blueprints";
import { discoverInstances } from "@/lib/engine";
import { pingMc } from "@/lib/mcping";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

/**
 * Live player metrics for every Conduit-managed instance, via Minecraft SLP.
 * No agent/plugin inside the servers — we status-ping each container directly.
 */
export async function GET() {
  try {
    const db = await getDB();
    const instances = (await discoverInstances()).filter(
      (i) => i.status === "running" && i.ip,
    );

    const rows = await Promise.all(
      instances.map(async (i) => {
        const task = db.tasks.find((t) => t.id === i.taskId);
        const bp = task ? blueprint(task.blueprintId) : undefined;
        const role = bp?.role ?? "generic";
        const port = bp?.port ?? 25565;
        const base = {
          vmid: i.vmid,
          taskId: i.taskId,
          taskName: task?.name ?? i.taskId,
          role,
          ip: i.ip,
          port,
        };
        // db/generic roles don't speak SLP — skip the ping
        if (role === "db" || role === "generic") {
          return { ...base, reachable: false, online: 0, max: 0, sample: [], version: "", latencyMs: 0 };
        }
        try {
          const p = await pingMc(i.ip!, port);
          return { ...base, reachable: true, ...p };
        } catch {
          return { ...base, reachable: false, online: 0, max: 0, sample: [], version: "", latencyMs: 0 };
        }
      }),
    );

    // Network player count = what the edge proxies report (avoids double-counting
    // a player who shows on both the proxy and the backend). Fall back to backends.
    const proxies = rows.filter((r) => r.role === "proxy" && r.reachable);
    const backends = rows.filter((r) => r.role !== "proxy" && r.reachable);
    const players = (proxies.length ? proxies : backends).reduce((n, r) => n + r.online, 0);
    const capacity = (proxies.length ? proxies : backends).reduce((n, r) => n + r.max, 0);

    return NextResponse.json({
      instances: rows,
      totals: {
        players,
        capacity,
        proxies: proxies.length,
        backends: backends.length,
        reachable: rows.filter((r) => r.reachable).length,
        instances: rows.length,
      },
    });
  } catch (e) {
    return NextResponse.json({ error: String(e) }, { status: 502 });
  }
}
